...
else
{
  ...
  //_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
   ...
   while(contenido!=1)
   {
      ...
      {
      //Initilization file used as 1st geometry
        command ="cp ";
      ...
      {
      //Starts randomnly generated cluster
         if(N_Simbolo_2>0)  // For bimetallic cases
         {
            ...
            else //pseudorandomly (cuts Au80 cluster)
            {
...
//      clus.move((x_max-x_min)/2.0-(clus.x_max()-clus.x_min())/2.0,(y_max-y_min)/2.0-(clus.y_max()-clus.y_min())/2.0,z_max-clus.z_min() );
//OLD      clus.move((x_max-x_min)/2.0+random_number(-dist,dist),(y_max-y_min)/2.0+random_number(-dist,dist),z_max-clus.z_min() );
      geometry_file.clear();
      geometry_file="geometry.tmp";
      clus.print_fhi(geometry_file);
...
else
{
  ...
  command="coordinaterejected"+m_str+".xyz";
  tag.clear();
  tag=" Iteration rejected "+m_str+" -----> Energy = "+E_str+" eV ";
  clus.print_xyz(command,tag);
  command.clear();

  command="mv coordinaterejected"+m_str+".xyz ./rejected";
  system(command.c_str());
  command.clear();

  command="mv output.out  ./rejected/outputrejected"+m_str+".out";
  system(command.c_str());
  command.clear();

  command="mv geometry.in ./rejected/geometryrejected"+m_str+".in";
  system(command.c_str());
  command.clear();
  m_str.clear();
  fail_counter=0;
